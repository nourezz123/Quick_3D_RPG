...
class HealthBar extends entity.Component {
  ...
  InitComponent() {
    ...
    if (healthComp && healthComp._params) {
      this._enemyLevel = healthComp._params.level || 1;
    }
  }
  ...
  _GetLevelColor(level) {
    // Color coding based on enemy level
    switch(level) {
      case 1:
        return new THREE.Color(0.0, 1.0, 0.0); // Green - Easy
      case 2:
        return new THREE.Color(1.0, 1.0, 0.0); // Yellow - Medium
      case 3:
        return new THREE.Color(1.0, 0.5, 0.0); // Orange - Hard
      case 4:
        return new THREE.Color(1.0, 0.0, 0.0); // Red - Very Hard
      case 5:
        return new THREE.Color(0.5, 0.0, 0.5); // Purple - Elite/Boss
      default:
        return new THREE.Color(0.8, 0.0, 0.8); // Bright Purple - Legendary
    }
  }
  ...
  Update(timeElapsed) {
    ...
    // Get base color from enemy level
    const baseColor = this._GetLevelColor(this._enemyLevel);
    
    // Interpolate to red as health decreases
    const _R = new THREE.Color(1.0, 0, 0);
    const c = baseColor.clone();
    c.lerpHSL(_R, 1.0 - this._animHealth); // Lerp towards red as health drops
